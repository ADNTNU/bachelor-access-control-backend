...
import no.ntnu.gr10.bacheloraccesscontrolbackend.administrator.dto.AdministratorListDto;
...
public class AdministratorService {
  ...
  /**
   * Constructor for AdministratorService.
   *
   * @param administratorRepository        the repository for managing administrators
   * @param administratorCompanyRepository the repository for managing
   *                                       administrator-company relations
   * @param passwordEncoder                the password encoder for hashing passwords
   * @param companyService                 the service for managing companies
   * @param sendGridService                the service for sending emails
   * @param jwtTokenProvider               the JWT token provider for generating
   *                                       and verifying tokens
   * @param passwordPolicyService          the service for validating passwords against policy
   */
  @Autowired
   ...
   *
   * <p>This method retrieves an Administrator entity from the
   * database based on the provided username.
   ...
   *
   * <p>This method retrieves an Administrator entity from the database
   ...
   *
   * <p>This method retrieves a list of AdministratorCompany entities from the database
   * based on the provided administrator IDs and company ID.
   ...
   */
  public List<AdministratorCompany> getAdministratorCompaniesByAdministratorIdsAndCompanyId(
          List<Long> administratorIds, long companyId
  ) {
    return administratorCompanyRepository
            .findByAdministratorIdsAndCompanyId(administratorIds, companyId);
  }
   ...
   */
  public AdministratorCompany getAdministratorCompanyByAdministratorIdAndCompanyId(
          long administratorId, long companyId
  ) throws AdministratorCompanyNotFoundException {
    return administratorCompanyRepository
            .findByAdministratorIdAndCompanyId(administratorId, companyId)
            .orElseThrow(() ->
                    new AdministratorCompanyNotFoundException("AdministratorCompany not found"));
  }

  /**
   * Gets a response with list of administrator-company relations by company ID.
   *
   * <p>This method retrieves a paginated list of AdministratorCompany entities
   * from the database based on the provided company ID.
   ...
   * @return the {@link ListResponse} containing the list of AdministratorListDto
   *         entities and pagination details
   */
  public ListResponse<AdministratorListDto> getListOfAdministratorsByCompanyId(
          PaginatedCrudListRequest request
  ) {
    int pageIndex = Math.max(0, request.getPage() - 1);
    Pageable pageable = PageRequest.of(pageIndex, request.getSize());

    Page<AdministratorCompany> page = administratorCompanyRepository
            .findPageableByCompanyId(request.getCompanyId(), pageable);
   ...
   *
   * <p>This method first checks if the company exists and then creates a new
   * Administrator entity if it doesn't already exist.
   ...
   * </p>
   *
   * @param inviteAdministratorRequest the request object containing the email,
   *                                   company ID, and role of the administrator
   ...
   */
  @Transactional
  public void inviteAdministrator(InviteAdministratorRequest inviteAdministratorRequest)
          throws
    ...
    Administrator administrator = administratorRepository
            .findByEmail(inviteAdministratorRequest.getEmail())
            .orElse(
                    new Administrator(
                            inviteAdministratorRequest.getEmail(),
                            temporaryUuidForUsername,
                            // Temporary password before user sets up the account
                            encodedPassword,
                            // Temporary first name before user sets up the account
                            "temporaryFirstName",
                            // Temporary last name before user sets up the account
                            "temporaryLastName"
                    )
            );
   ...
   *
   * <p>This method retrieves the AdministratorCompany entity based on the
   * provided invite token if it is valid.
   ...
   */
  @Transactional
  public void acceptInvite(AcceptAdministratorInviteRequest acceptAdministratorInviteRequest)
          throws AdministratorCompanyNotFoundException, JwtException, IllegalStateException {
    AdministratorCompany administratorCompany =
            getAdministratorCompanyFromInviteToken(
                    acceptAdministratorInviteRequest.getInviteToken()
            );
   ...
   * <p>This method retrieves the AdministratorCompany entity based on the
   * provided invite token if it is valid.
   * It updates the administrator's information such as username,
   ...
   * </p>
   *
   * @param registerAndAcceptAdministratorInviteRequest the request object containing the
   *                                                    invite token, username, password,
   *                                                    first name, and last name
   * @throws AdministratorCompanyNotFoundException       if the AdministratorCompany is not found
   * @throws JwtException                                if the JWT token verification fails
   * @throws PasswordPolicyService.WeakPasswordException if the password does not meet the
   *                                                     policy requirements
   * @throws IllegalArgumentException                    if any of the required fields are
   *                                                     missing or invalid
   */
  @Transactional
  public void registerAdministratorAndAcceptInvite(
          RegisterAndAcceptAdministratorInviteRequest registerAndAcceptAdministratorInviteRequest
  ) throws
          AdministratorCompanyNotFoundException,
          JwtException,
          PasswordPolicyService.WeakPasswordException,
          IllegalArgumentException {
    AdministratorCompany administratorCompany =
            getAdministratorCompanyFromInviteToken(
                    registerAndAcceptAdministratorInviteRequest.getInviteToken()
            );
    ...
    passwordPolicyService
            .validatePassword(registerAndAcceptAdministratorInviteRequest.getPassword());
    String encodedPassword = passwordEncoder
            .encode(registerAndAcceptAdministratorInviteRequest.getPassword());
  ...
  private AdministratorCompany getAdministratorCompanyFromInviteToken(String token)
          throws AdministratorCompanyNotFoundException, JwtException {
    Pair<Long, Long> idPair = jwtTokenProvider.verifyInviteTokenAndGetCompanyAndAdminId(token);

    return administratorCompanyRepository
            .findByAdministratorIdAndCompanyId(idPair.getSecond(), idPair.getFirst())
            .orElseThrow(
                    () -> new AdministratorCompanyNotFoundException(
                            "AdministratorCompany not found"
                    )
            );
  }
   ...
   *
   * <p>This method retrieves the AdministratorCompany entity based on
   * the provided administrator ID and company ID.
   * It updates the details of the AdministratorCompany entity.
   * Finally, it saves the updated AdministratorCompany entity.
   * </p>
   *
   * @param updateAdministratorCompanyRequest the request object containing the
   *                                          company ID, enabled status, and role
   * @param administratorId                   the ID of the administrator to be updated
   * @throws AdministratorCompanyNotFoundException if the AdministratorCompany is not found
   * @throws IllegalStateException                 if no active administrator-company
   *                                               relations would remain after the update
   */
  @Transactional
  public void updateAdministratorCompany(
          UpdateAdministratorCompanyRequest updateAdministratorCompanyRequest,
          Long administratorId
  ) {
    AdministratorCompany administratorCompany =
            getAdministratorCompanyByAdministratorIdAndCompanyId(
                    administratorId,
                    updateAdministratorCompanyRequest.getCompanyId()
            );
    ...
    if (administratorCompany.isEnabled()
            ...
            && administratorCompanyRepository
            .countAdministratorCompanyByEnabledIsTrueAndAcceptedIsTrue() <= 1
    ) {
      throw new IllegalStateException(
              "Cannot disable the last enabled administrator-company relation"
      );
    }

    administratorCompany.setEnabled(updateAdministratorCompanyRequest.getEnabled());
    administratorCompany.setRole(
            AdministratorRole
                    .fromString(updateAdministratorCompanyRequest.getRole()));
   ...
   *
   * <p>This method retrieves a list of AdministratorCompany entities based on the
   * provided administrator IDs and company ID.
   * It deletes all matching AdministratorCompany entities from the database.
   * </p>
   *
   * @param deleteAdministratorCompanyRequest the request object containing the list of
   *                                          administrator IDs and company ID
   * @throws AdministratorCompanyNotFoundException if no matching AdministratorCompany
   *                                               entities are found
   * @throws IllegalStateException                 if no active administrator-company
   *                                               relations would remain after deletion
   */
  @Transactional
  public void deleteAdministratorCompany(
          DeleteAdministratorCompanyRequest deleteAdministratorCompanyRequest
  )
          throws AdministratorCompanyNotFoundException {
    List<AdministratorCompany> administratorCompanies =
            getAdministratorCompaniesByAdministratorIdsAndCompanyId(
                    deleteAdministratorCompanyRequest.getAdministratorIds(),
                    deleteAdministratorCompanyRequest.getCompanyId()
            );
    ...
    long enabledCountBeforeDeletion = administratorCompanyRepository
            .countAdministratorCompanyByEnabledIsTrueAndAcceptedIsTrue() - activeToDeleteCount;

    if (enabledCountBeforeDeletion <= 0) {
      throw new IllegalStateException(
              "No enabled administrator-company relations would remain after deletion"
      );
    }
   ...
   *
   * <p>This method retrieves the Company entity based on the provided company ID.
   ...
   */
  @Transactional
  public void addAdministratorToCompany(
          long companyId, Administrator administrator, AdministratorRole role
   ...
   *
   * <p>This method retrieves the Administrator entity based on the provided email.
  ...
  public void requestPasswordReset(String email)
          throws UsernameNotFoundException, InvalidKeyException, SendMailException {
    Administrator administrator = getByUsernameOrEmail(email);

    String token = jwtTokenProvider
            .generatePasswordResetToken(String.valueOf(administrator.getId()));
    String encodedToken = URLEncoder.encode(token, StandardCharsets.UTF_8);
   ...
   *
   * <p>This method verifies the password reset token and retrieves the administrator ID from it.
   * It validates the new password against the password policy and
   ...
   * @throws JwtException                                if the JWT token verification fails
   * @throws NumberFormatException                       if the administrator ID in the
   *                                                     token is not a valid number
   * @throws UsernameNotFoundException                   if the administrator is not found
   * @throws PasswordPolicyService.WeakPasswordException if the new password does not meet the
   *                                                     policy requirements
   */
